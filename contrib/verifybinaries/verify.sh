...
###   isocoincore.org and isocoin.org and compares them.
...
WORKINGDIR="/tmp/isocoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://isocoincore.org"
HOST2="https://isocoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="isocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'isocoin-',
...
if [ $? -ne 0 ]; then
   echo "isocoin.org failed to provide signature file, but isocoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "isocoin.org and isocoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Isocoin Core binary release signing key installed?"
   fi
