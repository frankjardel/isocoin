 ...
 * between isocoind, and isocoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", ISOCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", ISOCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/isocoin/isocoin>";
    const std::string URL_WEBSITE = "<https://isocoincore.org>";
...
/** Sanity checks
 *  Ensure that Isocoin is running in a usable environment with all
...
{
    // Make sure only a single Isocoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", ISOCOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if isocoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if isocoin is started while in a temporary directory.\n",
