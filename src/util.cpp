...
const char * const ISOCOIN_CONF_FILENAME = "isocoin.conf";
const char * const ISOCOIN_PID_FILENAME = "isocoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "isocoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Isocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Isocoin
    // Mac: ~/Library/Application Support/Isocoin
    // Unix: ~/.isocoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Isocoin";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ISOCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", ISOCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Isocoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Isocoin Core") == std::string::npos) {
